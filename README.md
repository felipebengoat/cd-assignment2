...
|--------|------|-------------|---------|
| `Filename` | String | Name of source `.txt` file | `letter_001.txt` |
| `Document` | String | Original text exactly as read from file | Full letter text |
| `Tokens` | String | Space-separated word tokens | `Dear John , I hope ...` |
| `Lemmas` | String | Space-separated lemmatized forms | `dear John , I hope ...` |
| `Parts_of_speech` | String | Space-separated universal POS tags | `ADJ PROPN PUNCT PRON VERB ...` |
| `Named_Entities` | String | Named entities in `text/LABEL` format, pipe-separated | `John Smith/PERSON\|New York/GPE\|1965/DATE` |
| `Token_count` | Integer | Total number of tokens in the document | `856` |

**Note:** The columns `Title` and `Text` from the original assignment requirements were not populated, as:
- Letters did not have formal titles
- No additional text preprocessing was performed beyond tokenization
